...
if (Get-Module eiSCSI) {
    Remove-Module eiSCSI

}

$fullpath = $installDirectory + '/eiSCSI/' 

$moduleInfo = Test-ModuleManifest -Path ./eiSCSI/eiSCSI.psd1
...
Copy-Item ./eiSCSI/* -Destination $fullfolder -Recurse
Get-ChildItem $fullfolder -Recurse | Unblock-File

Import-Module eiSCSI
